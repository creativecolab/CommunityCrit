...
class TasksSeeder extends Seeder
{
	...
	public function run()
	{
		// Convert data array into collection
		$tasks = collect( $this->data );
	...
	{
		// Convert task array into collection
		$task = collect( $task );

		// Create task
		$newTask = Task::create( $task->only( [ 'name', 'text', 'parent_id' ] )->toArray() );

		// Create subtasks if available
		if ( $task->has( 'subtasks' ) ) {
			$subtasks = $task->get( 'subtasks' );

			// Save all tasks at end to be more efficient
			$toCreate = [];

			// Determine if any other tasks need to be recursively called
			foreach ( $subtasks as $subtask ) {
				if ( isset( $subtask['subtasks'] ) ) {
					// Define parent ID for next level of recursion
					$subtask['parent_id'] = $newTask->id;

					// Recursively call this function to explore more levels of subtasks
			...
			// Create subtasks
			$newTask->subtasks()->createMany( $toCreate );
		}
	}
}
